...
type GameStoreActions = {
  ...
  determineOutcome: (playerId: PlayerId) => 'lose' | 'win' | 'push'; // This could return a string indicating win, lose, or push
...
export const useGameStore = create(
  immer<GameStore & GameStoreActions>((set, get) => ({
    ...
    dealToDealer: async () => {
      ...
      while (true) {
        await sleep(1000);
        const validCounts = calculateHand(get().dealer).validCounts;
        if (
    ...
    startGame: () => {
      const { shuffle, drawCard } = useDeckStore.getState();
      shuffle();
      set(state => {
    ...
    outcome: '',
    didGameStart: () => get().dealer.length > 0,
